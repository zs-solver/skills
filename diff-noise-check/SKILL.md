...
description: 判断 Git 改动在剥离空白、横线、Markdown 管道表格列对齐、Obsidian wikilink 同目录路径展开等之后是否仍有实质差异；支持相对 HEAD、暂存区、工作区及嵌套子仓库等比较方式。适用于用户询问 diff 有无实质修改、保存后是否仅格式变化、表格对齐是否算改动等场景。
---

# Diff 噪声检查

把「看起来像改了很多行」的 diff 压成一句话：**归一化之后，还有没有实质不同**。脚本做确定性归一（空白、横线、表格列对齐、wikilink 路径展开等），比人肉扫 diff 稳。

**脚本路径**（相对本仓库根）：`.claude/skills/diff-noise-check/scripts/diff_noise_check.py`

---

## 设计目标：清理 Obsidian 误触发噪声，保证提交前 diff 干净

本 skill 面向一种常见场景：Obsidian 或相关插件在浏览、整理文档时，可能自动改写已有 Markdown 文件中的 wikilink 路径、`.md` 后缀、表格对齐或空白格式，导致 Git 中出现大量非意图性的 tracked diff。

这些 diff 会污染提交前审查：文件看起来被修改了，但实际内容没有变化。人工逐个文件对照既慢又容易误判。

因此本 skill 的目标不是做工作区审计，而是对已有基准版本的 tracked diff 做确定性归一化，判断这些改动是否只是表示层噪声：

- 如果归一化后一致，说明这些 tracked diff 可以安全视为噪声，适合直接撤销，恢复提交前干净状态；
- 如果归一化后仍有差异，只输出真正的实质差异，供人工审查；
- 未跟踪文件没有基准版本，不属于 Obsidian 误改已有文件的 diff 噪声判断对象，只能作为额外状态提示。
...
- **有实质差异**（未加 `--brief`）：会多几段——**判定思路**（脚本在做什么）、**表格对齐/wikilink 展开类噪声的一行统计**（有几段、大约几行 raw，不展开原文）、**归一化后的 unified diff**（看实质改了啥；太长会截断）。需要脚本式/一行结果时用 `--brief`。
...
- Wikilink **指向不同目录**的同名文件：`[[a/name]]` → `[[b/name]]` 不会被归一化（目录不匹配文件自身所在路径）。只有当 wikilink 的目录部分以文件自身目录结尾时（Obsidian 短链接 → 全路径展开）才视为噪声。
